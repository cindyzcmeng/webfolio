...
- [x] Fixed white sharp-corner artifact poking past the photo's organic silhouette on hover: rounded just the two photo-side corners of the clip-path (`PHOTO_CORNER_RADIUS`, 32px) so they sit closer to the photo's own curve. (First attempt insetted the whole box height by 22px instead, which also cropped the text panel's title/last tag row — caught via screenshot, reverted, redone as corner-rounding only, no height change.)
